...
namespace Graph_lib {
    ...
    Shape::Shape() :
            lcolor(fl_color()),      // default get_lcolor for lines and characters
            ls(0),                   // default style
    ...
    void Shape::draw_lines() const {
        if (get_lcolor().visibility() && 1 < points.size())    // draw sole pixel?
    ...
    void Shape::draw() const {
        ...
        fl_color(lcolor.as_int());            // set get_lcolor
        fl_line_style(ls.style(), ls.width()); // set style
        draw_lines();
        fl_color(oldc);      // reset get_lcolor (to previous)
    ...
    void Open_polyline::draw_lines() const {
        if (fill_color().visibility()) {
            ...
            fl_color(get_lcolor().as_int());    // reset get_lcolor
        }
    ...
    void Rectangle::draw_lines() const {
        ...
        if (get_lcolor().visibility()) {    // lines on top of fill
            fl_color(get_lcolor().as_int());
    ...
    void Axis::draw_lines() const {
        Shape::draw_lines();
        notches.draw();  // the notches may have a different get_lcolor from the line
        label.draw();    // the label may have a different get_lcolor from the line
    }
